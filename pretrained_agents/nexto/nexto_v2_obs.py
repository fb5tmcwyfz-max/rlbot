...
# rlgym is only used for type hints here. The offsets/memory build of Opti does
# not ship rlgym.utils (rlgym 2.x drops it), so fall back to duck-typed aliases
# when it is unavailable. The obs math below is unchanged either way.
try:
    from rlgym.utils.gamestates import GameState, PlayerData
except Exception:  # pragma: no cover - depends on installed rlgym version
    GameState = Any
    PlayerData = Any
...
class Nexto_V2_ObsBuilder(BatchedObsBuilder):
